...
class Fun:
    ...
    def save_data(self, data: str, url: str) -> None:
        ...
        try:
            # 修复Unicode编码问题：使用json参数而不是data参数
            if isinstance(data, str):
                # 如果data是JSON字符串，先解析为字典
            ...
            # 如果JSON解析失败，尝试使用data参数但确保编码正确
            try:
                response = requests.post(url, data=data.encode('utf-8'), headers=headers, timeout=10)
                response.raise_for_status()
                print("记录上传成功！")
            except requests.RequestException as e2:
                print(f"记录上传失败: {str(e2)}")

    def convert_avi_to_mp4(self, input_path: str, output_path: str = './runs/video/output.mp4') -> Generator[
        float, None, None]:
